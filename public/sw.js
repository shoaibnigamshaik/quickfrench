...
const isStaticAsset = (url) => /\.(?:js|css|woff2?|png|jpg|jpeg|gif|svg|ico|webp)$/i.test(url.pathname);
...
self.addEventListener('fetch', (event) => {
  ...
  if (isNavigation(request)) {
    event.respondWith((async () => {
      try {
        ...
        const networkResp = preload || await fetchWithTimeout(request, 6000);
  ...
  if (isSameOrigin(url) && (isNextStatic(url) || isStaticAsset(url) || url.pathname.startsWith('/_next/image'))) {
    event.respondWith((async () => {
  ...
  if (isSameOrigin(url)) {
    event.respondWith((async () => {
      ...
      try {
        ...
        if (resp.ok && (resp.headers.get('content-type') || '').match(/^(application|text)\//)) {
